...
def _compute_true_roic(ticker_obj):
    ...
    try:
        ...
        for nm in ["Operating Income", "EBIT", "Ebit"]:
            ...
            if matches:
                op_inc_row = matches[0]
                break
        if op_inc_row is None:
            return None
        ...
        cash_use         = cash if cash is not None else 0
        invested_capital = equity + debt - cash_use
...
def build_screener_table(universe_df, yf_fundamentals, momentum_map):
    ...
    num_cols = [
        ...
        "MC% of Index", COL_RQ1, COL_RQ2, COL_RQ3, COL_RQ4,
        "Rev Growth% (CAGR)",
    ]
    for c in num_cols:
        if c in scr.columns:
...
# ─── KPI panel ────────────────────────────────────────────────────────────────
def render_sector_kpi_panel(scr, sector_sel, index_label):
    ...
    c1.markdown(_kpi("Sector Mkt Cap",  fmt_mc_inr(sector_mc), "Rs Lakh Cr"),
                unsafe_allow_html=True)
    ...
    pe_fwd_str = (
        ...
        else ("{:.1f}".format(med_pe) if pd.notna(med_pe) else "N/A")
    )
    c4.markdown(_kpi("Median P/E → Fwd", pe_fwd_str,
...
def render_screener_ui(scr, index_label):
    ...
    COLS = [
        ...
        COL_RQ1, COL_RQ2, COL_RQ3, COL_RQ4, "Rev Growth% (CAGR)",
        "Data Sources",
    ]
...
**Earn Traj:** (Forward EPS − Trailing EPS) / |Trailing EPS|. Positive = analysts expect earnings growth.
+0.20 = ~20% EPS improvement forecast. Range: −1.0 (severe decline) to +1.0 (strong recovery).

**MC% of Index:** This stock's market cap as % of total index market cap.

**Score:** Sector-adaptive weights — select a specific sector to see active weights in the analysis panel above.

**ROIC%:** True Return on Invested Capital = NOPAT / (Equity + Debt − Cash).
...
with pg1:
    ...
    has_fwd   = sum(1 for t in tickers50 if fd50.get(t, {}).get("fwd_pe")       is not None)
    has_peg   = sum(1 for t in tickers50 if fd50.get(t, {}).get("peg")          is not None)
    has_roic  = sum(1 for t in tickers50 if fd50.get(t, {}).get("roic")         is not None)
...
with pg2:
    ...
    with col_r2:
        ...
        if st.button("Refresh Cache", key="n500_refresh"):
            ...
            st.rerun()
    with col_t2:
        st.caption("Last loaded: {} · 6hr cache".format(
            datetime.now().strftime("%I:%M %p")))
    ...
    if load_500 or st.session_state.get("n500_loaded", False):
        ...
        has_fwd   = sum(1 for t in tickers500 if fd500.get(t, {}).get("fwd_pe")       is not None)
        has_peg   = sum(1 for t in tickers500 if fd500.get(t, {}).get("peg")          is not None)
        has_roic  = sum(1 for t in tickers500 if fd500.get(t, {}).get("roic")         is not None)
...
with pg3:
    ...
    with col_pick:
        selected = st.multiselect(
            ...
            default=all_tickers_for_comp[:3] if len(all_tickers_for_comp) >= 3
                    else all_tickers_for_comp,
            max_selections=5,
            key="comp_select",
        )
    with col_hint:
    ...
    if len(selected) < 2:
        ...
        comp_tickers = tuple(
            (s if s.endswith(".NS") else s + ".NS") for s in selected)
        ...
        header_row = (
            "<tr><th style='padding:10px 14px;background:#1a1a2e;color:#aaa;"
            "text-align:left;border-bottom:2px solid #2a2a4a;'>Metric</th>"
            + header_cells + "</tr>"
        )
        ...
        for col in radar_df.columns:
            ...
            col_min = series.min(); col_max = series.max()
        ...
        for i, m in enumerate(radar_metrics):
            ...
            anchor = ("middle" if abs(math.cos(angle)) < 0.3
        ...
        comp_export = pd.DataFrame({
            m[0]: {t.replace(".NS", ""): raw_vals[m[0]].get(t.replace(".NS", ""))
                   for t in comp_tickers}
