...
"""Evaluate how predictive the Indian screener scores are and train a first ML model.
...
def load_data():
    ...
    archive = archive.sort_values(["run_timestamp"]).drop_duplicates(
        subset=["run_date", "Ticker", "index"], keep="last"
    )
...
def compute_forward_returns(archive: pd.DataFrame, prices: pd.DataFrame) -> pd.DataFrame:
    ...
    for label in HORIZONS:
        ...
        archive[f"nifty50_ret_{label}"] = np.nan
    ...
    # NIFTY50 benchmark returns for each run
    if "NIFTY50" in price_series:
        bench_s = price_series["NIFTY50"]
        idx = bench_s.index
        bench_dates = np.asarray(archive["run_date"])
        bench_pos = idx.searchsorted(bench_dates, side="right") - 1
        bench_pos = np.clip(bench_pos, 0, len(idx) - 1)
        bench_now = bench_s.iloc[bench_pos].values
        for label, h in HORIZONS.items():
            bench_fwd_pos = bench_pos + h
            valid = bench_fwd_pos < len(idx)
            bench_rets = np.full(len(archive), np.nan, dtype=float)
            bench_rets[valid] = bench_s.iloc[bench_fwd_pos[valid]].values / bench_now[valid] - 1.0
            archive[f"nifty50_ret_{label}"] = bench_rets
    else:
        for label in HORIZONS:
            archive[f"nifty50_ret_{label}"] = np.nan
    ...
    for label in HORIZONS:
        ...
        for factor in FACTORS:
            if factor not in archive.columns:
                continue
            ic_values = []
            for _, run_df in archive.groupby(["run_date", "run_id", "index"]):
...
def top_n_backtest(archive: pd.DataFrame, factor: str, n: int = 20) -> pd.DataFrame:
    ...
    records = []
    for (run_date, run_id, index_name), run_df in archive.groupby(["run_date", "run_id", "index"]):
        ...
        for label in HORIZONS:
            ret_col = f"ret_{label}"
            bench_col = f"nifty50_ret_{label}"
            if ret_col not in top.columns:
                continue
            mean_ret = top[ret_col].mean()
            bench_vals = run_df[bench_col].dropna()
            bench_ret = bench_vals.iloc[0] if bench_col in run_df.columns and len(bench_vals) > 0 else np.nan
            records.append({
                "run_date": run_date,
                "run_id": run_id,
                "index": index_name,
                ...
                "nifty50_return": bench_ret,
                "active_return": mean_ret - bench_ret if not np.isnan(bench_ret) else np.nan,
...
def _numeric_features(df: pd.DataFrame) -> list[str]:
    ...
    for c in df.columns:
        if c in EXCLUDE_COLS:
            continue
        if c.startswith("ret_") or c.startswith("nifty50_ret_") or c.startswith("spy_ret_"):
            continue
...
def train_ml_model(archive: pd.DataFrame, horizon: str):
    ...
    bench_col = f"nifty50_ret_{horizon}"
    ...
    for run_date, run in test_df.groupby("run_date"):
        ...
        bench_ret = run[bench_col].dropna().iloc[0] if bench_col in run.columns else np.nan
...
def main():
    ...
    print("\nTop-20 equal-weight backtest (active return vs NIFTY50)")
    backtest_records = []
    for factor in ["Score", "Conviction Score", "Quality Score", "Momentum Score"]:
        if factor not in archive.columns:
            continue
        ...
        summary = bt.groupby("horizon").agg(
            ...
            mean_nifty50_return=("nifty50_return", "mean"),
    ...
    drop_cols = (
        ["run_timestamp", "run_id", "run_date", "index"]
        + [f"ret_{h}" for h in HORIZONS]
        + [f"nifty50_ret_{h}" for h in HORIZONS]
    )
    feature_cols = [c for c in archive.columns if c not in drop_cols]
    ml_cols = feature_cols + [f"ret_{h}" for h in HORIZONS] + [f"nifty50_ret_{h}" for h in HORIZONS]
