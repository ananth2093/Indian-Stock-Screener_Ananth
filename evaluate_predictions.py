...
"""Evaluate how predictive the Indian screener scores are using the saved archive.
...
def top_n_backtest(archive: pd.DataFrame, factor: str, n: int = 20) -> pd.DataFrame:
    ...
    for (run_date, run_id, index_name), run_df in archive.groupby(["run_date", "run_id", "index"]):
        ...
        for label in HORIZONS:
            ...
            bench_ret = run_df[bench_col].dropna().iloc[0] if bench_col in run_df.columns else np.nan
            records.append({
                "run_date": run_date,
                "run_id": run_id,
                "index": index_name,
